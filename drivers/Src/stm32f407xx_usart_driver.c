...
void USART_ReceiveData(USART_Handle_t *pUSARTHandle, uint8_t *pRxBuffer,
	...
	for (uint32_t i = 0; i < length; i++) {
		...
		//Check the USART_Wordlengthgth to decide whether we are going to receive 9bit of data in a frame or 8 bit
		if (pUSARTHandle->USART_Config.USART_WordLength == USART_WORDLENGTH_9) {
			...
			if (pUSARTHandle->USART_Config.USART_ParityControl
				...
				*((uint16_t*) pRxBuffer) = (pUSARTHandle->pUSARTx->USART_DR
						& (uint16_t) 0x01FF);
			...
			if (pUSARTHandle->RxLen > 0) {
				...
				if (pUSARTHandle->USART_Config.USART_WordLength
					...
					if (pUSARTHandle->USART_Config.USART_ParityControl
						...
						//read 8 bits from DR
						*pUSARTHandle->pRxBuffer =
								(uint8_t) (pUSARTHandle->pUSARTx->USART_DR
										& (uint8_t) 0xFF);
